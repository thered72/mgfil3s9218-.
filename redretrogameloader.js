(async function () {
    ...
    async function filesmissing(file) {
        ...
        console[minifiedFailed ? "warn" : "error"](
            "Failed to load " + file +
            " because the minified files are likely missing.\n" +
            "Fix options:\n" +
            "1. Download latest release from EmulatorJS GitHub\n" +
            "2. Extract emulator.min.zip into data/ (recommended)\n" +
            "3. Build manually with npm"
        );
    ...
    if (
        ...
        try {
            let path =
                typeof EJS_paths !== "undefined" && typeof EJS_paths[language] === "string"
                    ? EJS_paths[language]
